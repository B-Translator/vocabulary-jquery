...
OAuth2.Token = function (settings) {
    ...
    var _settings = {
        ...
        proxy_url: 'https://example.org/user/oauth2_login?proxy=true',
    ...
    this.erase = function () {
        ...
        if (_settings.auth_flow == 'proxy') {
            var url = _settings.proxy_url;
            base_url = url.substring(0, url.indexOf('/', 14));
            window.open(base_url + '/user/logout');
        }
    ...
    var _getFromProxy = function () {
        var login = window.open(_settings.proxy_url, 'login');
