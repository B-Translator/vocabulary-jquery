...
OAuth2.Token = function (settings) {
    ...
    var _settings = {
        ...
        proxy_login: 'https://example.org/user/proxy_login',
        proxy_logout: 'https://example.org/user/proxy_logout',
    ...
    this.erase = function () {
        ...
        if (_settings.auth_flow == 'proxy') {
            var logout = window.open(_settings.proxy_logout);
            if (logout) {
                setTimeout(function () {
                    logout.close();
                }, 800);
    ...
    var _refreshExisting = function (get_new) {
        if (_settings.auth_flow == 'proxy') {
            // In 'proxy' authorization flow token cannot be refreshed.
    ...
    var _getFromProxy = function () {
        ...
        if ($.isFunction(_settings.openLoginWindow)) {
            _settings.openLoginWindow(_settings.proxy_login, _getTokenFromProxy);
        }
        else {
            var win = window.open(_settings.proxy_login);
            _getTokenFromProxy(_settings.proxy_login, win);
        }
    };
